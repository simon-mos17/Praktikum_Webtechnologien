  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === 4) {
      ...
      // (404 = Not Found -> User nicht gefunden -> OK)
        ...
        console.log(
          "Server: Username ist frei (404). Starte Registrierung über /register."
        );
      ...
      // Andere Statuscodes -> allgemeiner Fehler (z.B. Serverproblem)
        ...
        markInvalid(usernameInput);
        alert(
          "Fehler beim Prüfen des Benutzernamens... (Status " +
            xhr.status +
...
function updateFriendOptions(filterText) {
  ...
  if (filter.length > 0) {
    // Wenn der Nutzer etwas getippt hat (filter.length > 0): nochmal zusätzlich nach Text filtern
    candidates = candidates.filter((name) =>
      name.toLowerCase().includes(filter)
    ); // Für jeden Namen prüfen: kommt der Filtertext drin vor?
  }
...
function handleAddFriend() {
  ...
  if (!valid) {
    alert(
      "Ungültiger Freundesname. Bitte einen gültigen Nutzer wählen, " +
